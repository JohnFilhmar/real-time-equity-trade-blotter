...
|---|---|---|---|
| `npm test` | Unit and route tests in all three workspaces: contract, service and route suites against an in-memory repository, and on the client the cache patching, formatting, flash, keyboard focus, date range and ticket form logic. Rebuilds the shared contract first, so a pull cannot leave the suites reading stale output | nothing | 489 pass: 60 shared, 241 backend, 188 frontend |
