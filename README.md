...
|---|---|---|---|
| `npm test` | Unit and route tests in all three workspaces: contract, service and route suites against an in-memory repository, and on the client the cache patching, formatting, flash, keyboard focus, date range and ticket form logic | nothing | 479 pass: 60 shared, 231 backend, 188 frontend |
| `npm run test:integration` | Repository, refresh-token and positions tests against real Postgres and Redis, including the append-only trigger | the compose stack | 40 pass |
