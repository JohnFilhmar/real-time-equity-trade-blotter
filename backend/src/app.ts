 ...
 *
 * Order is load-bearing: hardening and parsing first, then routes, then the 404, then the error
 * handler last. A route mounted after `not_found_handler` would never be reached, which is the
 * trap the original scaffold left behind.
 *
...
export function create_app(dependencies: AppDependencies): Express {
  ...
  app.use('/api/trades', create_trade_router(dependencies.trade_service));
