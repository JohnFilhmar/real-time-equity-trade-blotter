...
describe('health routes', () => {
  it('reports liveness without touching the database', async () => {
    const response = await request(create_app({ health_probe: unreachable, cors_origins })).get('/health');
  ...
  it('reports ready when the database answers', async () => {
    const response = await request(create_app({ health_probe: reachable, cors_origins })).get('/ready');
  ...
  it('reports 503 when the database is unreachable', async () => {
    const response = await request(create_app({ health_probe: unreachable, cors_origins })).get('/ready');
...
describe('unmatched routes', () => {
  it('returns the standard error shape rather than a bare 404', async () => {
    const response = await request(create_app({ health_probe: reachable, cors_origins })).get('/does-not-exist');
