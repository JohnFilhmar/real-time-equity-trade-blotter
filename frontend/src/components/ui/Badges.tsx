...
export function StatusBadge({ status }: { status: TradeStatus }): ReactNode {
  ...
  return (
    <span className={`whitespace-nowrap rounded-[3px] px-[7px] py-[2.5px] font-mono text-[9.5px] font-semibold tracking-[.07em] ${tone}`}>
      {status}
    </span>
  );
}
...
export function VersionPill({ version }: { version: number }): ReactNode {
  ...
  return (
    <span
      className="cursor-help rounded-[3px] bg-warn-bg px-[5px] py-[2.5px] font-mono text-[9.5px] font-semibold text-warn shadow-[inset_0_0_0_1px_var(--warn_edge)]"
...
export function Chip({
  ...
  return (
    <span className="inline-flex h-[26px] items-center gap-[6px] whitespace-nowrap rounded-[13px] border border-rule bg-glass-soft px-[9px] text-[11.5px] text-text-2">
      {label}
      ...
      {onRemove !== undefined ? (
        <button
          ...
          className="grid h-[14px] w-[14px] place-items-center rounded-full text-muted hover:bg-loss-bg hover:text-loss"
        >
